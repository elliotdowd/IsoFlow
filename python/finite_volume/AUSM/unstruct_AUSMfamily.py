...
def unstruct_AUSM( mesh, state, parameters, gas ):
    ...
    Phibound[:,0] = np.zeros( len(mesh.bdry_ind[0]) )
    ...
    while n <= parameters.iterations and max(state.res[n-1+np.int(n<10),:]) > parameters.tolerance: 
        ...
        state.ht = thermo.calc_rho_et(state.p, state.Q[:,0], state.u, state.v, gas.gamma / \
                                      state.Q[:,0] + state.p/state.Q[:,0] ) / state.Q[:,0] + \
                                      state.p / state.Q[:,0]
        ...
        Phibound[:,3] = thermo.calc_rho_et(state.pbound, state.Qbound[:,0], state.ubound, state.vbound, gas.gamma / \
                                           state.Qbound[:,0] + state.pbound/state.Qbound[:,0] ) / state.Qbound[:,0] + \
        ...
        # update state vector
        state.Q = state.Qn + 1e-10 * state.residual / mesh.dV4
